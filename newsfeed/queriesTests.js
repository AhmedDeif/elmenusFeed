                ...
                if (err) {
                    ...
                    db.query('CREATE (:Cuisine { name:{np} })', params = {
                        ...
                        if (err) {
                            ...
                            db.query('CREATE (s:Scores { followsScore:5, reviewScore:3 , likesDishScore:7 ,hasCuisineScore:6 , addPhotoScore:6 , yum_yuckScore:3 , shareRestaurantScore:2 ,shareDishScore:2 , sharePhotoScore:2 , favouritesScore:8 , likeCuisineScore:3  })', params = {}, function(err, results) {
                                if (err) {
                                    console.error('Error');
                                    throw err;
                                } else {
        ...
        function verify() {
            db.query('optional match (:User {name:{e1p}})-[rel:FOLLOWS]->(:User {name:{e2p}}) return rel', params = {
                ...
                if (err) {
                    ...
                    var res = results.map(function(result) {
                        return result['rel'];
                    });
        ...
        function verify() {
            db.query('optional MATCH (n:User { email:{ep}}),(r:Restaurant { name:{rp}}), (n) -[Rev:Review { title:{tp} , body:{bp} }]-> (r) return Rev', params = {
                ...
                if (err) {
                    ...
                    var Rev = results.map(function(result) {
                        return result['Rev'];
                    });
                    should.exist(Rev[0]);
                    done();
        ...
        function verify() {
            db.query('optional MATCH (u:User {email: {ep}}), (u1:User {email: {ep1}}) , (d:Dish {dish_name: {dnp}}), (u)-[relLikes:LIKES_DISH {likes:FALSE}]->(d), (u)-[z:FOLLOWS]-(u1) return relLikes, z.totalScore', params = {
                ...
                if (err) {
                    ...
                    var relLikes = results.map(function(result) {
                        return result['relLikes'];
                    });
                    var totalScore = results.map(function(result) {
                        return result['z.totalScore'];
                    });
        ...
        function verify() {
            db.query('optional MATCH (n:User { email:{ep} }),(r:Restaurant { name:{rp} }), (photo:Photo { url : {url}}), (n) -[photoRel:addPhoto]->(photo)-[inRel:IN]->(r) return photoRel, photo, inRel', params = {
                ...
                if (err) {
                    ...
                    var photoRel = results.map(function(result) {
                        return result['photoRel'];
                    });
                    var photo = results.map(function(result) {
                        return result['photo'];
                    });
                    var inRel = results.map(function(result) {
                        return result['inRel'];
                    });
        ...
        function verify() {
            db.query('optional MATCH (u1:User{email:{e1p}}),(u2:User{email:{e2p}}), (u1)-[f:FOLLOWS{ numberOfVisits :0 , totalScore :5}]->(u2) return f.numberOfVisits, f.totalScore', params = {
                ...
                if (err) {
                    ...
                    var numberOfVisits = results.map(function(result) {
                        return result['f.numberOfVisits'];
                    });
                    var totalScore = results.map(function(result) {
                        return result['f.totalScore'];
                    });
...
describe('A user can unfollow a followed user', function() {
    it('A user should be unfollowed', function(done) {
        ...
        function verify() {
            db.query('match (n:User)-[f:FOLLOWS]->(u:User) where n.email ={e1p} and u.email = {e2p} return count(f) ', params = {
                e1p: 'doola1@mail.com',
                e2p: 'doola2@mail.com'
            }, function(err, results) {
                if (err) {
                    ...
                    var numberOfFollows = results.map(function(result) {
                        return result['count(f)'];
                    });
...
describe('A user can share a photo', function() {
    it('new photo added with relation to user', function(done) {
        ...
        function test() {
            db.query(queries.UserSharesPhotoQuery, params = {
                ep: 'doolatest@mail.com',
                url: 'photo@url.com'
            }, function(err, results) {
                if (err) {
        ...
        function verify() {
            db.query('optional MATCH (d:User{email:{ep}})-[:SHARE_PHOTO]->(p:Photo{url:{url}}) return d,p;', params = {
                ep: 'doolatest@mail.com',
                url: 'photo@url.com'
            }, function(err, results) {
                if (err) {
...
describe('A restaurant can be created', function() {
    it('A node representing a restaurant should be made', function(done) {
        ...
        function verify() {
            db.query('match (n:Restaurant) where n.name ={r1} return n.name ', params = {
                r1: 'Pizza Hut'
            }, function(err, results) {
                if (err) {
                    console.error('Error');
...
describe('A User can be created', function() {
    it('', function(done) {
        ...
        function verify() {
            db.query('match (n:User) where n.email ={u1} return n.email; ', params = {
                u1: 'TestUser'
            }, function(err, results) {
                if (err) {
                    console.error('Error');
                    ...
                    var userName = results.map(function(result) {
                        return result['n.email'];
                    });
...
describe('The cost of Score should change', function() {
    it('set favourit cost to 7', function(done) {
        initialize();

        function initialize() {
            db.query("CREATE (s:Scores { followsScore:5 , reviewScore:4 , likesDishScore:5 ,hasCuisineScore:6, addPhotoScore:8, yum_yuckScore:6, shareRestaurantScore:2 ,shareDishScore:4 , sharePhotoScore:1, favouritesScore:2, likeCuisineScore:0})", params = {}, function(err, results) {
                if (err) {
                    console.error('Error');
                    throw err;
                } else test();
        ...
        function verify() {
            db.query('optional MATCH (n:User { email:{ep} }),(c:Cuisine{name:{cp}}), (n) -[l:LIKECUISINE]->(c) return l.score', params = {
                ...
                if (err) {
                    ...
                    var scoreAfterAdd = results.map(function(result) {
                        return result['l.score'];
                    });
        ...
        function verify() {
            db.query('optional MATCH (n:User { email:{ep} }),(c:Cuisine{name:{cp}}), (n) -[l:LIKECUISINE]->(c) return l.score', params = {
                ...
                if (err) {
                    ...
                    var scoreAfterFav = results.map(function(result) {
                        return result['l.score'];
                    });
