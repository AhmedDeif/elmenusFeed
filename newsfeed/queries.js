...
exports.UserSharesDish = function(UserEmail, DishName) {
    db.query("MATCH (user:User {email: {ep}}), (dish:Dish {dish_name: {dn}}) , (s:Scores) CREATE (user)-[:SHARE_DISH {score:s.shareDishScore, timestamp: TIMESTAMP()}]->(dish)", params = {
        ep: UserEmail,
        dn: DishName
    }, function(err, results) {
        if (err) throw err;
...
//favourites between the two nodes.
...
exports.getRecommendationsQuery = "";z
exports.getLatestActionTime = function (callback) {
    db.query("MATCH (:User)-[r]->() RETURN MAX(r.created_at);", function (err, results) {
        if (err)
        {
